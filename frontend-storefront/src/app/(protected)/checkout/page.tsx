...
const checkoutSchema = z.object({
  ...
  paymentMethod: z.enum(['COD', 'BANK_TRANSFER'], { message: 'Please select a payment method' }),
});
          ...
          <div className="w-full lg:w-[70%]">
            <form id="checkout-form" onSubmit={handleSubmit(onSubmit)} className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <div className="mb-4">
                ...
                <input {...register('fullName')} className={`w-full rounded-lg border p-3 outline-none focus:ring-1 ${errors.fullName ? 'border-red-500' : 'border-gray-300 focus:border-[#FF4742] focus:ring-[#FF4742]'}`} placeholder="Enter your full name" />
              ...
              <div className="mb-4">
                ...
                <input {...register('phoneNumber')} className={`w-full rounded-lg border p-3 outline-none focus:ring-1 ${errors.phoneNumber ? 'border-red-500' : 'border-gray-300 focus:border-[#FF4742] focus:ring-[#FF4742]'}`} placeholder="Enter your phone number" />
                {errors.phoneNumber && <p className="mt-1 text-xs text-red-500">{errors.phoneNumber.message}</p>}
              </div>

              <div className="mb-6">
                <label className="mb-1 block text-sm font-medium text-gray-700">Detailed Address <span className="text-red-500">*</span></label>
                <textarea {...register('shippingAddress')} rows={3} className={`w-full rounded-lg border p-3 outline-none focus:ring-1 ${errors.shippingAddress ? 'border-red-500' : 'border-gray-300 focus:border-[#FF4742] focus:ring-[#FF4742]'}`} placeholder="House number, street, ward/commune, district..." />
                {errors.shippingAddress && <p className="mt-1 text-xs text-red-500">{errors.shippingAddress.message}</p>}
              </div>

              <h2 className="mb-4 mt-8 text-lg font-bold">Payment Method <span className="text-red-500">*</span></h2>
          ...
          <div className="w-full lg:w-[30%]">
            <div className="sticky top-24 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-2 flex justify-between text-gray-600">
                ...
                <span>{SHIPPING_FEE.toLocaleString()} VND</span>
              </div>
              <div className="mt-4 flex justify-between border-t border-gray-100 pt-4 text-lg font-bold text-gray-900">
                <span>Total</span>
                <span className="text-[#FF4742]">{(totalCartPrice + SHIPPING_FEE).toLocaleString()} VND</span>
              </div>
